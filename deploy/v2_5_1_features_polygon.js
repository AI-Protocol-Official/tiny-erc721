// run: npx hardhat deploy --network mumbai --tags v2_5_1_features_polygon
...
module.exports = async function({deployments, getChainId, getNamedAccounts, getUnnamedAccounts}) {
	...
	// ERC721
	{
		...
		const requested_features = toBN(FEATURE_TRANSFERS | FEATURE_TRANSFERS_ON_BEHALF | FEATURE_OWN_BURNS);
		if(!features.eq(requested_features)) {
			// update the features as required
			const receipt = await deployments.execute(
				"PolygonDigitalTwin", // name: string,
				{from: A0}, // options: TxOptions,
				"updateFeatures", // methodName: string,
				requested_features.toString(10), // ...args: any[]
			);
			console.log("PolygonDigitalTwin.updateFeatures(%o): %o", requested_features.toString(2), receipt.transactionHash);
		}
	}
...
module.exports.tags = ["v2_5_1_features_polygon", "features_polygon", "v2_5_1"];
